...
def create_lexicon(
    file_name: str = "rnr-examples.csv", column: str = "text", top_n_acronyms: int = 200
):
    ...
    top_acronyms = sorted(acronyms.items(), key=lambda x: x[1], reverse=True)[
        :top_n_acronyms
    ]
    ...
    for i, (acronym, _) in enumerate(tqdm(top_acronyms)):
        ...
        for page in pages:
            try:
                ...
                acronym_wiki.append(
                    {"acronym": acronym, "summary": summary, "tags": tags}
                )
